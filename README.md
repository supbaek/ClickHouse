...
### OS/VM 준비
- [x] ClickHouse 전용 VM 1대 준비 (고정 IP, hostname, 시간 동기화).
- [x] OS 패키지 업데이트 및 기본 도구 설치.

### ClickHouse 설치
- [x] ClickHouse 공식 문서에서 설치 방법 선택 (rpm/deb 패키지 또는 quick install 스크립트).
- [x] `clickhouse-server`, `clickhouse-client` 설치.
- [x] `clickhouse-server` 서비스 기동 및 `systemctl status`로 정상 기동 확인.
- [x] 방화벽에서 9000(TCP), 8123(HTTP) 포트 확인/허용.

### 디렉토리·파일 구조 파악
- [x] `/etc/clickhouse-server/` 구조 확인 (`config.xml`, `users.xml` 등).
- [x] `/var/lib/clickhouse/` 데이터 디렉토리 구조 확인.
- [x] `/var/log/clickhouse-server/` 로그 파일 위치 확인.

### 기본 SQL 및 접속
- [x] `clickhouse-client`로 접속 후 `SELECT 1;` 실행.
- [x] `SHOW DATABASES;`, `CREATE DATABASE testdb;`, `USE testdb;` 실행.

### 테스트 테이블·쿼리
- [x] `testdb`에 간단한 MergeTree 테이블 생성.
  - 예: `user_actions(user_id UInt64, action String, action_time DateTime) ENGINE = MergeTree() ORDER BY (user_id, action_time);`
- [x] `INSERT` 10~100건 넣고 `SELECT`, `WHERE`, `GROUP BY`, `ORDER BY`, `LIMIT` 쿼리 실행.

### 기본 설정 훑어보기
- [x] `config.xml`에서 `listen_host`, `tcp_port`, `http_port` 항목 확인.
- [x] `users.xml`에서 기본 유저 `default` 설정 확인 (필요시 비밀번호·쿼리 제한 간단히 설정).
